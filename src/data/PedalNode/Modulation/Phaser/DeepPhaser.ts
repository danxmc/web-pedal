export interface DeepPhaserOptions extends AudioNodeOptions {
  depth: number;
  rate: number;
  resonance: number;
}

export class DeepPhaserNode extends GainNode {
  // Parameters
  depth: AudioParam;
  rate: AudioParam;
  ...
  private _filter3Node: BiquadFilterNode;
  private _resonanceNode: GainNode;
  ...
  constructor(audioContext: BaseAudioContext, options: DeepPhaserOptions) {
    super(audioContext);
    ...
    this._filter3Node = new BiquadFilterNode(audioContext, {
      type: 'allpass',
      frequency: 1100,
    });
    this._resonanceNode = new GainNode(audioContext, {
      gain: options.resonance || 0.5,
    });
    this._delayNode = new DelayNode(audioContext, { delayTime: 0.001 });
    ...
    this.resonance = this._resonanceNode.gain;
    ...
    for (let key in options) {
      switch (key) {
        ...
        case 'rate':
          ...
          break;
        case 'resonance':
          this.resonance.value = options[key];
          break;
      }
    }
    ...
    this._depthRangeNode.connect(this._filter3Node.frequency);
    this._filter3Node
      .connect(this._delayNode)
      .connect(this._resonanceNode)
      .connect(this._filter1Node);
    this._lfoNode.start();
  }
  ...
  public setResonance(targetValue: number | string) {
    this.resonance.value = parseFloat(<string>targetValue);

    this._resonanceNode.gain.value = this.resonance.value;
  }
}
