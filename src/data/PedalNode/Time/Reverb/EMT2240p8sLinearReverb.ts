...
export interface EMT2240p8sLinearReverbOptions extends AudioNodeOptions {
  wet: number;
  level: number;
}

export class EMT2240p8sLinearReverbNode extends GainNode {
  // Parameters
  wet: AudioParam;
  level: AudioParam;
  ...
  constructor(audioContext: BaseAudioContext, options: EMT2240p8sLinearReverbOptions) {
    super(audioContext, {
      gain: 1,
    });
    ...
    getReverbBuffer(IMPULSE_RESPONSE.REVERB_FILES.emt2240p8sLinear, audioContext).then(
      (decodedData) => (this._convolverNode.buffer = decodedData)
    );
