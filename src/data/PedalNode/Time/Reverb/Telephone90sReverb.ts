...
export interface Telephone90sReverbOptions extends AudioNodeOptions {
  wet: number;
  level: number;
}

export class Telephone90sReverbNode extends GainNode {
  // Parameters
  wet: AudioParam;
  level: AudioParam;
  ...
  constructor(audioContext: BaseAudioContext, options: Telephone90sReverbOptions) {
    super(audioContext, {
      gain: 1,
    });
    ...
    getReverbBuffer(IMPULSE_RESPONSE.REVERB_FILES.telephone90s, audioContext).then(
