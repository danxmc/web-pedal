...
export interface CarRadioReverbOptions extends AudioNodeOptions {
  wet: number;
  level: number;
}

export class CarRadioReverbNode extends GainNode {
  // Parameters
  wet: AudioParam;
  level: AudioParam;
  ...
  constructor(audioContext: BaseAudioContext, options: CarRadioReverbOptions) {
    super(audioContext, {
      gain: 1,
    });
    ...
    getReverbBuffer(IMPULSE_RESPONSE.REVERB_FILES.carRadio, audioContext).then(
