...
export interface WalkmanReverbOptions extends AudioNodeOptions {
  wet: number;
  level: number;
}

export class WalkmanReverbNode extends GainNode {
  // Parameters
  wet: AudioParam;
  level: AudioParam;
  ...
  constructor(audioContext: BaseAudioContext, options: WalkmanReverbOptions) {
    super(audioContext, {
      gain: 1,
    });
    ...
    getReverbBuffer(IMPULSE_RESPONSE.REVERB_FILES.walkman, audioContext).then(
