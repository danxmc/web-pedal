...
export interface HallReverbOptions extends AudioNodeOptions {
  wet: number;
  level: number;
}

export class HallReverbNode extends GainNode {
  // Parameters
  wet: AudioParam;
  level: AudioParam;
  ...
  constructor(audioContext: BaseAudioContext, options: HallReverbOptions) {
    super(audioContext, {
      gain: 1,
    });
    ...
    getReverbBuffer(IMPULSE_RESPONSE.REVERB_FILES.hall, audioContext).then(
