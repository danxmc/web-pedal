...
export function switchNodeAtIndexInAudioNodesChain(
  audioNode: AudioNode,
  index: number,
  ...
  // Disconnect/Connect new node at index
  if (index === chainLength - 1) {
    if (chainLength === 1) {
      // inputNode.disconnect();
      disconnectAudioNode(inputNode, audioNodesChain[index]);

      // audioNodesChain[index].disconnect();
      disconnectAudioNode(audioNodesChain[index], audioContext.destination);
      connectAudioNode(inputNode, audioNode, audioContext.destination);
    } else {
      console.log('a');
      // audioNodesChain[index - 1].disconnect();
      // audioNodesChain[index].disconnect();
      disconnectAudioNode(audioNodesChain[index - 1], audioNodesChain[index]);
      // disconnectAudioNode(audioNodesChain[index], audioContext.destination);
      connectAudioNode(
        audioNodesChain[index - 1],
        audioNode,
    ...
    // audioNodesChain[index].disconnect()
    // disconnectAudioNode(audioNodesChain[index], audioNodesChain[index + 1]);
    // console.log('c');

    // maybe add index + 1?
    connectAudioNode(inputNode, audioNode, audioNodesChain[index + 1]);
    // console.log('d');
  } else {
    // disconnectAudioNode(audioNodesChain[index - 1], audioNodesChain[index]);
    // disconnectAudioNode(audioNodesChain[index], audioNodesChain[index + 1]);
...
export function getAudioNodeIndexesInChain(
  nodeSplits: Array<number>,
  pedalPosition: number
): Array<number> {
  const potsInPedalCount = nodeSplits[pedalPosition];
  const startIndex = nodeSplits
    .slice(0, pedalPosition)
    .reduce((accum, currentVal) => accum + currentVal, 0);

  const audioNodeIndexesInChain = Array.from(
    { length: potsInPedalCount },
    (_value, potIndex) => startIndex + potIndex
  );
  return audioNodeIndexesInChain;
}

export function connectAudioNodeInChain(
  potsValues: Array<PotUIType>,
  inputNode: AudioNode,
  audioContext: AudioContext
): PotUIType[] {
  return potsValues.map((potValues, i, arrPots) => {
    if (potValues.node) {
      const size = arrPots.length - 1;
      // If only node
      if (size === 0) {
        connectAudioNode(inputNode, potValues.node, audioContext.destination);
      } else if (i === size) {
        // If last node
        const inN = arrPots[i - 1].node ?? inputNode;
        connectAudioNode(inN, potValues.node, audioContext.destination);
      } else if (i === 0) {
        // If first node
        const outN = arrPots[i + 1].node ?? audioContext.destination;
        connectAudioNode(inputNode, potValues.node, outN);
      } else {
        // If middle node
        const inN = arrPots[i - 1].node ?? inputNode;
        const outN = arrPots[i + 1].node ?? audioContext.destination;
        connectAudioNode(inN, potValues.node, outN);
      }
    }
    return potValues;
  });
...
export function disconnectAudioNodeInChain(
  potsValues: Array<PotUIType>,
  inputNode: AudioNode,
  audioContext: AudioContext
): PotUIType[] {
  return potsValues.map((potValues, i, arrPots) => {
    if (potValues.node) {
      ...
      if (i === size) {
        // If last node
        disconnectAudioNode(potValues.node, audioContext.destination);
      } else if (i === 0 || size === 0) {
        // If first node || If only node
        disconnectAudioNode(inputNode, potValues.node);
      } else {
        // If middle node
        const prevN = arrPots[i - 1].node ?? inputNode;
        disconnectAudioNode(prevN, potValues.node);
      }
    }
    return potValues;
  });
